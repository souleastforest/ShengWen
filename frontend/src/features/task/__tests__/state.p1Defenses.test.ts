/**
 * P7 迁移：P1 防线加固（安全 + 一致性守卫）→ features/task/state.ts 域
 * 原文件 useTaskViewModel.p1Defenses.test.ts（任务部分；[P1-5] quality
 * payload 断言随 upload 域迁至 features/upload/__tests__/state.payload.test.ts）。
 *
 * 覆盖：
 * 1. [P1-2] 内容合并守卫统一：WS 重连全量对账（onopen → fetchTasks 列表合并 +
 *    selectTask(currentTask)，现由订阅层 status==='open' watch 驱动）后，
 *    已加载的完整 summary/transcript 不得被轻量列表项清空或回退为截断版；
...
describe('P1 防线加固：task 域（useTaskViewModel → state.ts 迁移）', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
  })
  ...
  describe('[P1-2] 内容合并守卫统一（WS 重连对账）', () => {
    it('WS 重连全量对账后，已加载的完整 summary/transcript 被保留（不被轻量列表项清空/截断）', async () => {
      ...
      // 断线重连：onopen 触发全量对账（订阅层 watch(status==='open')）——
      // fetchTasks 列表 + selectTask(currentTask)。
    ...
    it('[I-1] 双开标签页：断线期间任务内容被另一标签页重置（latest_modified_at 变化），重连对账不得保留陈旧内容', async () => {
      ...
      task.activeTab.value = 'transcript'
      await flushPromises()
      expect(task.selectedTask.value?.summary).toBe('旧总结')
      expect(task.selectedTask.value?.latest_modified_at).toBe('T0')
    ...
    it('WS task_update 广播保持内容权威：re-transcribe 的 "" 重置必须传播到 selectedTask', async () => {
      ...
      expect(task.selectedTask.value?.summary).toBe('')
    ...
    it('WS task_update 广播携带新的完整内容时以广播为准（权威来源覆盖旧内容）', async () => {
      ...
      expect(task.selectedTask.value?.transcript).toBe('新完整转录')
      expect(task.selectedTask.value?.summary).toBe('新完整总结')
  ...
  describe('[P1-4] summary tab 按需加载补齐', () => {
    ...
    it('摘要被后端重置为空串（""）后，已完成任务的 summary tab 自动重载完整版（断线自愈）', async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith('/tasks/task-a?include_content=true')
      expect(task.selectedTask.value?.summary).toBe('重新总结后的新内容')
