/**
 * 对抗性测试：deleteTask tombstone（P2-D）
 *
...
describe('useTaskViewModel deleteTask tombstone（P2-D）', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    vi.clearAllMocks()
  ...
  it('D1 删除成功后，同任务 task_update 广播不复活（tombstone 命中）', async () => {
    ...
    const deleted = await viewModel.deleteTask('t1')
    expect(deleted).toBe(true)
    expect(viewModel.tasks.value).toHaveLength(0)

    // 在途广播"复活攻击"：删除后到达的 task_update（列表 unshift 分支）
    wsInstance!.onmessage?.({ data: taskUpdateBroadcast({ ...TASK_T1, status: 'TRANSCRIBING' }) })
    expect(viewModel.tasks.value).toHaveLength(0) // 不复活
  ...
  it('D3 删除失败不记录 tombstone：广播仍可合并（"删除成功后"条件）', async () => {
    ...
    expect(viewModel.tasks.value.map((t) => t.id)).toEqual(['t1']) // 删除失败，任务仍在

    wsInstance!.onmessage?.({
      data: taskUpdateBroadcast({ ...TASK_T1, status: 'TRANSCRIBING', progress: 10 }),
    })
    expect(viewModel.tasks.value.map((t) => t.id)).toEqual(['t1']) // 广播正常合并（未被墓碑屏蔽）
