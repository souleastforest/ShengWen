...
- 2026-06-30: Reinstalled runtime dependencies (torch 2.5.1+cu121, transformers 4.57.6, librosa, soundfile, accelerate, diffusers, vibevoice editable). Fixed huggingface-hub version conflict (pinned <1.0 for transformers 4.57 compat). Created `VibeVoiceAsrTranscriber` with lazy model loading, timestamp parsing, and cancel/progress support. Updated `settings_manager.py` to support `transcriber_type` switching between `fast_whisper` and `vibe_voice_asr` (constructor param, runtime state, build_transcriber_kwargs, update_settings, get_settings).
