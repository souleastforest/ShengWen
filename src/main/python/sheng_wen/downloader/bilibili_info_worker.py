...
from .bilibili_headers import build_bilibili_http_headers
...
class BilibiliInfoWorker(Worker):
    ...
    async def process_task(self, payload: Any):
        ...
        try:
            # B 站反爬需要浏览器级请求头，优先使用全局 SESSDATA Cookie
            sessdata = ""
            try:
                ...
                logger.warning(
                    f"[{self.name}] 读取全局 B 站 Cookie 失败，仅使用浏览器请求头"
                )
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                video_info = {
                    ...
                    "duration_string": self._format_duration(
                ...
                logger.info(
                    f"  时长: {video_info['duration_string']} ({video_info['duration']}秒)"
                )
